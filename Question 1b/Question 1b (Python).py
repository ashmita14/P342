...
f=lambda x: -x-math.cos(x) #given function
a=float(input('Lower Guess : '))
b=float(input('Upper Guess : '))

#bracketing the given function
a, b, chk_brac = non_lineareqns.bracketing(f, a, b, 10)
if chk_brac==True:
...
#bisection method
name_bi='Absolute Error Bisection (1b).txt'
...
#regula falsi method
name_rf='Absolute Error Regula Falsi (1b).txt'
...
nr=float(input('Guess for Newton Raphson : '))
name_nr='Absolute Errors Newton Raphson (1b).txt'
file=open(name_nr, 'w') #'w' ==> write, clearing up file contents before making new entry
root_nr, chk_nr=non_lineareqns.newton_raphson(f, nr, 100, name_nr)
if chk_nr==True:
